...
function load_string_viewer() {
    ...
    $('.dump-string-format-plain, .dump-string-format-html').click(function () {
        ...
        var textDiv = $(this).closest('.dump-string').find('.dump-string-text, .dump-string-html');
        if ($(this).hasClass('dump-string-format-plain')) { //Mostrar texto plano
            $(textDiv).text($(textDiv).html()).attr('class','dump-string-text');
        } else { //Mostrar HTML
            $(textDiv).html($(textDiv).text()).attr('class','dump-string-html');
        }
    });
}
